...
export function Summary({items = [], onOrderSubmit}: {items: any[], onOrderSubmit: any}) {
  const totalPrice = items.reduce(
      (prev, curr) => prev + (curr.price * curr.count), 0);
  const [order, setOrder] = useState({
    email: "", address: "", postcode: ""
  });
  const handleEmailInputChanged = (e: any) => setOrder(
      {...order, email: e.target.value})

  const handleAddressInputChanged = (e: any) => setOrder(
      {...order, address: e.target.value})

  const handlePostcodeInputChanged = (e: any) => setOrder(
    ...
    <form>
      <div className="mb-3">
        ...
        <input type="email" className="form-control mb-1"
               onChange={handleEmailInputChanged} value={order.email}
               id="email"/>
      </div>
      <div className="mb-3">
